...
class redux_auth {
     ...
     * @author Mathew
     * */
    public function register($username, $password, $email, $fullname = NULL, $roleId = 3) {
        $email_activation = $this->ci->config->item('email_activation');
        $email_folder = $this->ci->config->item('email_templates');

        if (!$email_activation) {
            return $this->ci->redux_auth_model->register($username, $password, $email, $fullname, $roleId);
        } else {

            $register = $this->ci->redux_auth_model->register($username, $password, $email, $fullname, $roleId);
...
//            return $this->ci->email->send();
    ...
    public function list_user(){
        return $this->ci->redux_auth_model->list_user();
    }
    /**
     * logout
     *
